...
CXXFLAGS="-std=c++20 -g -Wall -Wextra -O3 -I./include -I. -I./out -pthread -Wno-cast-function-type -static-libstdc++ -static-libgcc"
...
cmd_build() {
  ...
  if ! docker image inspect "$DOCKER_IMAGE" >/dev/null 2>&1; then
    die "Docker image '${DOCKER_IMAGE}' not found. Run: cross-build.sh setup"
  fi
  ...
  if [ ! -f "${OUTPUT_DIR}/${bin_name}" ]; then
    die "Build failed: ${OUTPUT_DIR}/${bin_name} not found."
  fi
